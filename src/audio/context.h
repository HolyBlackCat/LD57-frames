    ...
    class Context
    {
      ...
      public:
        ...
        Context(attribute_list_t attributes)
        {
            ...
            EM_FINALLY_ON_THROW{alcCloseDevice(data.device);};
            ...
            if (std::tie(lib_major, lib_minor) < std::tie(required_major, required_minor))
                throw std::runtime_error(fmt::format("The OpenAL dynamic library is too old. Expected at least version {}.{}.", required_major, required_minor));
            ...
            EM_FINALLY_ON_THROW{alcDestroyContext(data.context);};
