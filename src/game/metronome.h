...
class Metronome
{
  ...
  public:
    ...
    {
        tick_len = std::uint64_t(Clock::TicksPerSecond() / freq);
    }
    ...
    bool Tick(std::uint64_t delta)
    {
        ...
        // Compensate.
        if (std::abs(std::int64_t(accumulator - tick_len)) < tick_len * comp_th)
        {
            int dir;
            if (comp_dir)
                dir = -comp_dir;
            ...
            comp_dir += dir;
            accumulator += std::uint64_t(tick_len * comp_amount * dir);
        }
        ...
        {
            // Do tick.
            if (max_ticks && accumulator > tick_len * std::uint64_t(max_ticks))
            {
                accumulator = tick_len * std::uint64_t(max_ticks);
                lag = true;
            }
